...
RSpec.describe BasePresenter do
  # Create a subclass to avoid polluting the BasePresenter class
  class Presenter < BasePresenter; end
  let(:presenter) { Presenter.new('fake', { somthing: 'cool' }) }
  ...
  describe '#initialize' do
    ...
    it 'sets the "params" variable with { somthing: "cool" }' do
      expect(presenter.params).to eq({ somthing: 'cool' })
    end
  ...
  describe '#as_json' do
    it 'allows the serialization of "data" to json' do
      presenter.data = { somthing: 'cool' }
      expect(presenter.to_json).to eq '{"somthing":"cool"}'
    end
  end
